...
def start_practice():
    ...
    try:
        ...
        try:
            ...
            # Start proctoring for practice interviews
            proctoring_response = requests.post(
                f"{request.url_root}api/proctoring/start",
                json={'interview_session_id': session.id},
                headers={'Content-Type': 'application/json'}
            )
            print(f"Proctoring start response: {proctoring_response.status_code}")
        except Exception as e:
            print(f"Proctoring start error: {e}")
...
def start_adaptive_interview(session_id):
    ...
    try:
        ...
        try:
            ...
            print(f"Proctoring started for session {session.id}: {proctoring_response.status_code}")
        except Exception as e:
            print(f"Proctoring start error: {e}")
...
def complete_adaptive_interview(session_id):
    ...
    try:
        ...
        try:
            import requests
            proctoring_response = requests.post(
                f"{request.url_root}api/proctoring/stop",
                json={'interview_session_id': session.id},
                headers={'Content-Type': 'application/json'}
            )
            print(f"Proctoring stopped for session {session.id}: {proctoring_response.status_code}")
        except Exception as e:
            print(f"Proctoring stop error: {e}")
