...
class StudentProfile(db.Model):
    ...
    # Consolidated skills and resume data (JSON format)
    skills = db.Column(db.Text)  # JSON: [{'name', 'category', 'proficiency', 'years', 'confidence'}]
    technical_skills = db.Column(db.Text)  # JSON: technical skills only
    soft_skills = db.Column(db.Text)  # JSON: soft skills only
    
    # Resume parsing results
    resume_file = db.Column(db.String(255))
    resume_text = db.Column(db.Text)  # Parsed resume content
    parsed_contact = db.Column(db.Text)  # JSON: contact info
    parsed_education = db.Column(db.Text)  # JSON: education details
    parsed_experience = db.Column(db.Text)  # JSON: work experience
    parsed_projects = db.Column(db.Text)  # JSON: projects
