...
class InterviewReportPDF:
    ...
    def generate_candidate_report(self, candidate_data, feedback_data, output_path=None):
        ...
        # Security Report
        if 'security_report' in feedback_data:
            security = feedback_data['security_report']
            security_header = Paragraph("🛡️ Security & Integrity Report", self.styles['SectionHeader'])
            story.append(security_header)
            
            security_data = [
                ['Metric', 'Value'],
                ['Integrity Level', security.get('integrity_level', 'N/A')],
                ['Integrity Score', f"{security.get('integrity_score', 0)}/100"],
                ['Tab Switches', str(security.get('tab_switches', 0))],
                ['Total Violations', str(security.get('total_violations', 0))],
                ['Multiple Faces Detected', f"{security.get('face_analysis', {}).get('multiple_faces_detected', 0)} times"],
                ['No Face Detected', f"{security.get('face_analysis', {}).get('no_face_detected', 0)} times"],
                ['Total Face Checks', str(security.get('face_analysis', {}).get('total_checks', 0))]
            ]
            
            security_table = Table(security_data, colWidths=[3*inch, 3*inch])
            ...
            if issues:
                issues_header = Paragraph("Security Issues:", self.styles['CustomBody'])
                story.append(issues_header)
                for issue in issues:
                    bullet = Paragraph(f"• {issue}", self.styles['CustomBody'])
                    story.append(bullet)
                story.append(Spacer(1, 10))
            
            # Security Recommendation
            rec_text = Paragraph(f"<b>Recommendation:</b> {security.get('recommendation', 'N/A')}", 
                               self.styles['CustomBody'])
